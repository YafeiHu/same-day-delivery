 ...
 * @author hyf
 */
public class ViewProductRateJPanel extends javax.swing.JPanel {
    ...
    public ViewProductRateJPanel(JPanel userProcessContainer, Enterprise enterprise) {
        initComponents();
        ...
        for (Product p : directory) {
            if (p.getReviews() != null) {
                for (Review r : p.getReviews()) {
                    System.out.println(r.getStarForSupplier());
                    System.out.println(r);
                }
                Object row[] = new Object[5];
                row[0] = p;
                row[1] = p.getProductId();
                row[2] =p.getAvailable(); 
                row[3] = p.getPrice();
                row[4] = p.rate();
    ...
    private void popTree() {
        ...
        ArrayList<ProductDirectory> directorys = new ArrayList<>();
        for (Organization o : enterprise.getOrganizationDirectory().getOrganizationList()) {
            if (o instanceof WarehouseOrganization) {
                for (ProductDirectory pd : ((WarehouseOrganization) o).getProductCatalog().getProductDirectorys()) {
                    for (ProductSubDirectory psd : pd.getSubDirectoryList()) {
                        for (Product p : psd.getProductList()) {
    ...
    private void viewProdjButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_viewProdjButton2ActionPerformed
        ...
        Product o = (Product) productJTable.getValueAt(selectedRow, 0);
        ViewProductJPanel vjp = new ViewProductJPanel(userProcessContainer, o);
